...
export class AsterClient extends BaseExchangeClient {
  ...
  constructor() {
    // Use Aster WebSocket endpoint
    super('Aster', exchangeConfig.aster.wsUrl);
  }

  async connect(): Promise<void> {
    // Build combined stream URL with all subscriptions for Aster
    if (this.subscribedSymbols.length > 0) {
      const streams: string[] = [];

      for (const symbol of this.subscribedSymbols) {
        // Add USDT suffix for Aster futures trading pairs
        const asterSymbol = `${symbol}USDT`.toLowerCase();
        streams.push(`${asterSymbol}@trade`);
        streams.push(`${asterSymbol}@depth20@100ms`);

        for (const interval of this.subscribedIntervals) {
          streams.push(`${asterSymbol}@kline_${interval}`);
        }
      }

      // Use combined stream endpoint for Aster
      this.url = `${exchangeConfig.aster.wsUrl}/stream?streams=${streams.join('/')}`;
    } else {
      // Use simple WebSocket endpoint if no streams specified
      this.url = `${exchangeConfig.aster.wsUrl}/ws`;
    }
  ...
  async subscribe(symbols: string[], intervals: string[] = ['1m']): Promise<void> {
    ...
    logger.info(`Aster subscribed to symbols: ${symbols.join(', ')} with intervals: ${intervals.join(', ')}`);
  }

  async unsubscribe(symbols: string[]): Promise<void> {
    if (!this.isConnected) {
      throw new Error('Aster client not connected');
    }
    ...
    logger.info(`Aster unsubscribed from symbols:`, symbols);
  }

  sendHeartbeat(): void {
    // Aster's WebSocket handles ping/pong automatically
  ...
  protected handleMessage(message: AsterMessage): void {
    ...
    if (message.stream && message.data) {
      ...
      if (streamParts.length >= 2) {
        ...
        if (streamType === 'trade') {
          this.handleTrade(message.data as AsterTradeData, symbol);
        } else if (streamType && streamType.startsWith('depth')) {
          this.handleDepthUpdate(message.data as AsterDepthData, symbol);
    ...
    if (message.e === 'trade' && message.s) {
      this.handleTrade(message as AsterTradeData, message.s);
      return;
    }

    // Handle direct depth update messages
    if (message.e === 'depthUpdate' && message.s) {
      this.handleDepthUpdate(message as AsterDepthData, message.s);
      return;
    }
  ...
  private handleTrade(data: AsterTradeData, symbol: string): void {
  ...
  private handleDepthUpdate(data: AsterDepthData, symbol: string): void {
    // Remove USDT suffix to normalize symbol
    const normalizedSymbol = symbol.replace('USDT', '');
  ...
  public async requestOrderbookSnapshot(symbol: string): Promise<void> {
    ...
    this.sendMessage({
      method: 'SUBSCRIBE',
      params: [`${asterSymbol}@depth20@100ms`],
      id: Date.now(),
    });
  }
  ...
  private handleKline(data: any, symbol: string, interval: string): void {
    ...
    const k = data.k;
    // Aster kline data format (Binance-compatible):
