...
class TradingDataBackend {
  ...
  private setupEventHandlers(): void {
      ...
      try {
        ...
        if (snapshot.bids.length > 0 && snapshot.asks.length > 0) {
          ...
          await chartProcessor.processTickData({
            ...
            size: '0', // Size doesn't matter for mid-price ticks
            side: 'buy',
    ...
    this.hyperliquidClient.on('trades', async (trades) => {
      try {
        logger.info(`Hyperliquid: Received ${trades.length} trades - ${trades[0]?.symbol} @ ${trades[0]?.price}`);
    ...
    // Hyperliquid candle handler
    this.hyperliquidClient.on('candle', async (candle: any) => {
      try {
        // Publish to Redis for SSE streaming
        ...
        logger.debug(`Hyperliquid candle published: ${candle.exchange}:${candle.symbol}:${candle.interval}`);
      } catch (error) {
        logger.error('Failed to process Hyperliquid candle:', error);
      }
    });
    ...
    // Aster candle handler
    this.asterClient.on('candle', async (candle: any) => {
      try {
        // Publish to Redis for SSE streaming
        await redisPublisher.publishCandle({
          exchange: candle.exchange,
        ...
        logger.debug(`Aster candle published: ${candle.exchange}:${candle.symbol}:${candle.interval}`);
      } catch (error) {
        logger.error('Failed to process Aster candle:', error);
      }
    });
    ...
    chartProcessor.on('candleUpdated', async (update) => {
      ...
      // Publish real-time candle updates to Redis (legacy - keep for backward compatibility)
      try {
        // Map exchange names: 'hyperliquid' -> 'hl', 'aster' -> 'aster'
    ...
    chartProcessor.on('candleCompleted', async (update) => {
      ...
      try {
        ...
        logger.error('Failed to publish completed candle to Redis:', error);
      }
    });
  }
...
const backend = new TradingDataBackend();
